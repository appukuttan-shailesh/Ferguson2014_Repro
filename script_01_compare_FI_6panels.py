...
plt.plot(list(data_Brian2_SA.keys()), list([x["initial_f"] for x in data_Brian2_SA.values()]), '-.g', label="Brian2", linewidth=2.0)
plt.plot(list(data_Neuron_SA.keys()), list([x["initial_f"] for x in data_Neuron_SA.values()]), ':m', label="Neuron", linewidth=1.0)
plt.title("Strongly adapting model - Initial frequency")
...
plt.plot(list(data_Brian2_SA.keys()), list([x["final_f"] for x in data_Brian2_SA.values()]), '-.g', linewidth=2.0)
plt.plot(list(data_Neuron_SA.keys()), list([x["final_f"] for x in data_Neuron_SA.values()]), ':m', linewidth=1.0)
plt.title("Strongly adapting model - Final frequency")
# plt.legend(['Original-Initial', 'Original-Final'], loc='best')
plt.xlabel("$I_{applied} (pA)$")
plt.ylabel("AP frequency (Hz)")
plt.xlim(-50.0, 300.0)
...
plt.plot(list(data_Neuron_WA1.keys()), list([x["initial_f"] for x in data_Neuron_WA1.values()]), ':m', linewidth=1.0)
plt.title("Weakly adapting model #1 - Initial frequency")
# plt.legend(['Original-Initial', 'Original-Final'], loc='best')
plt.xlabel("$I_{applied} (pA)$")
plt.ylabel("AP frequency (Hz)")
plt.xlim(-50.0, 350.0)
...
plt.plot(list(data_Neuron_WA1.keys()), list([x["final_f"] for x in data_Neuron_WA1.values()]), ':m', linewidth=1.0)
plt.title("Weakly adapting model #1 - Final frequency")
# plt.legend(['Original-Initial', 'Original-Final'], loc='best')
plt.xlabel("$I_{applied} (pA)$")
plt.ylabel("AP frequency (Hz)")
plt.xlim(-50.0, 350.0)
...
plt.plot(list(data_Neuron_WA2.keys()), list([x["initial_f"] for x in data_Neuron_WA2.values()]), ':m', linewidth=1.0)
plt.title("Weakly adapting model #2 - Initial frequency")
plt.xlabel("$I_{applied} (pA)$")
plt.ylabel("AP frequency (Hz)")
plt.xlim(-50.0, 350.0)
plt.ylim(0, 50)
...
plt.plot(list(data_Neuron_WA2.keys()), list([x["final_f"] for x in data_Neuron_WA2.values()]), ':m', linewidth=1.0)
plt.title("Weakly adapting model #2 - Final frequency")
plt.xlabel("$I_{applied} (pA)$")
plt.ylabel("AP frequency (Hz)")
plt.xlim(-50.0, 350.0)
plt.ylim(0, 50)
...
lines, labels = [sum(lol, []) for lol in zip(*lines_labels)][:4]
#specify order of items in legend
# order = [0, 4, 1, 5, 2, 6, 3, 7]
order = [0, 1, 2, 3]
fig.legend([lines[idx] for idx in order], [labels[idx] for idx in order], loc='lower center', bbox_to_anchor=(0.5, 0), fancybox=True, shadow=True, ncol=4)
